...
1. **Skeleton Source** — manually provide anything from a short thematic seed to granular established world lore. Auto-Generate remains intentionally conservative: it derives a generalized backdrop from recent chat without copying named characters, party members, locations, factions, or plot events.
2. **Generate Skeleton** — factions, locations, NPCs, conflicts as Day 0 baseline in `{prefix}_Skeleton`. You can enable **Source from Existing Lorebooks** and select books whose entries should be treated as established canon during generation; selecting none uses all non-skeleton lorebooks. Enable **Only use entities explicitly mentioned** to prevent extrapolation; the category counts are then ignored. Edit the generated skeleton afterward in the native lorebook UI for full customization.
